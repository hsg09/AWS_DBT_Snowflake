...
create or replace transient table ANALYTICS.DEV_HARNEKSINGH_marts.dim_products
...
WITH products AS (
    SELECT * FROM ANALYTICS.DEV_HARNEKSINGH_staging.stg_raw__products
),
...
categories AS (
    SELECT
        category_name,
        parent_category
    FROM ANALYTICS.DEV_HARNEKSINGH_seeds.product_categories
),
...
alter table ANALYTICS.DEV_HARNEKSINGH_marts.dim_products cluster by (category, is_active);
